...
_INTENT_SIGNALS: list[IntentSignal] = [
    IntentSignal(
        ...
        patterns=[
            ...
            r"compara\s+(los|las|el|la)\s+\d*\s*(mejores?)?\s*(aceite|filtro|bater[íi]a|llanta|neumático|repuesto)",
            r"(mejores?)\s+(aceites|filtros|baterías|llantas|neumáticos|repuestos)",
            r"(aceite|filtro|batería|llanta|neumático)\s+(sintético|mineral|semi)",
            r"(5w-?30|5w-?40|10w-?30|0w-?20)\s+(vs|versus|o|para|en)",
            r"(API\s+(SP|SN|SM|SL))\s+(vs|versus|o|para)",
            r"(ACEA\s+[A-C]\d?)\s+(vs|versus|o|para)",
            r"(OEM|aprobación)\s+(vs|versus|o|para)",
        ],
    ),
    IntentSignal(
    ...
    IntentSignal(
        ...
        patterns=[
            ...
            # Product comparison patterns
            r"(mejor|mejores)\s+(aceite|filtro|bater[íi]a|llanta|neumático)",
            r"compar[aá]\s+(aceite|filtro|bater[íi]a|llanta|neumático)",
            r"(cu[aá]l|cual)\s+(aceite|filtro|bater[íi]a|llanta)\s+(compro|comprar|elijo)",
            r"cuánto\s+(cuesta|vale|gasto)\s+(el\s+)?(aceite|filtro|bater[íi]a|llanta)",
            r"(aceite|filtro|bater[íi]a|llanta|repuesto)\s+(para|compatible|servicio)",
            r"cu[aá]l?\s+es\s+(el\s+)?(mejor|óptimo)\s+(aceite|filtro|bater[íi]a|llanta)",
        ],
    ),
]
...
_AUTOMOTIVE_DOMAIN_KEYWORDS: set[str] = {
    _strip_accents(kw) for kw in {
        ...
        "diferencial", "eje", "turbo", "turboalimentado",
        "cilindros", "cilindrada", "caballos", "hp", "torque",
        "potencia", "compresion", "encendido", "inyeccion",
        "inyector", "inyectores", "bomba de combustible",
        "valvulas", "correa", "cadena de distribucion",
        "cooler", "intercooler", "wastegate",
        # ── Brakes ──
        "freno", "frenos", "pastillas", "discos", "balatas",
        "calipers", "caliper", "cilindro de freno",
        "líquido de frenos", "liquido de frenos", "dot3", "dot4", "dot5",
        # ── Steering & suspension ──
        "direccion", "volante",
        "rótulas", "rotulas", "brazos de suspension",
        "bujes", "bushings", "barra estabilizadora",
        # ── Electrical ──
        "bateria", "baterias", "alternador", "arranque",
        "fusible", "fusibles", "relé", "rele",
        "cableado", "chicote", "bujia", "bujias",
        "bobina de encendido", "sensor", "sensores",
        # ── Cooling ──
        "radiador", "termostato", "bombas",
        "manguera", "mangueras", "thermostat",
        "anticongelante", "refrigerante", "coolant",
        "water pump", "ventilador",
        # ── Lubricants & fluids ──
        "aceite", "aceites", "lubricante", "lubricantes",
        "sintetico", "semisintetico", "mineral",
        "5w30", "5w-30", "5w40", "5w-40", "10w30", "10w-30",
        "10w40", "10w-40", "0w20", "0w-20", "15w40", "15w-40",
        "20w50", "20w-50", "75w90", "75w-90", "75w140",
        "viscosidad", "indice de viscosidad",
        "api sp", "api sn", "api sl", "api sm",
        "acea", "ilsac", "gf-5", "gf-6",
        "aprobacion oem", "aprobaciones oem",
        "cambio de aceite", "filtro de aceite",
        "aceite de motor", "aceite de transmision",
        "aceite de caja", "aceite de diferencial",
        "aceite de frenos", "aceite hidraulico",
        "fluido de transmision", "fluido de caja",
        " ATF", "atf",
        "fluid brake",
        # ── Filters ──
        "filtro", "filtros",
        "filtro de aire",
        "filtro de combustible", "filtro de cabina",
        "filtro de habitaculo", "filtro de transmision",
        "filtro hidraulico", "filtro de aceite hidraulico",
        "aire acondicionado", "a/c", "clima",
        # ── Tires & wheels ──
        "llantas", "neumaticos", "gomas",
        "rim", "rines", "aleacion",
        "presion de llantas", "presion",
        "rotacion de llantas",
        "desgaste", "desecho", "cubierta",
        "radial", "diagonal", "tubeless",
        # ── Fuel & energy ──
        "combustible", "gasolina", "diesel", "electrico",
        "hibrido", "etanol", "gnv", "glp",
        "gas natural", "lng",
        "octanaje", "ron", "azel", "additivada",
        "inflamable", "premium", "regular",
        # ── Exhaust & emissions ──
        "escape", "tubo", "catalizador", "mofle",
        "emisiones", "contaminacion",
        "particulas", "dpf", "fap",
        "eGR", "egr", "SCR", "scr", "adblue", "def",
        # ── Body & interior ──
        "puertas", "parabrisas",
        "faros", "headlight", "luces", "led", "halogeno",
        "retrovisor", "cristales", "vidrios",
        "asientos", "tapiceria", "consola",
        "pantalla", "infotainment", "apple carplay", "android auto",
        "sonido", "bocinas", "audio",
        # ── Safety ──
        "airbag", "cinturon", "seguridad",
        "isofix", "abs", "control de estabilidad",
        "camara", "sensores de estacionamiento",
        "frenos abs", "distribucion de frenado",
        # ── Vehicle types ──
        ...
        # ── Performance & tuning ──
        "rendimiento", "aceleracion",
        "mejora", "modificacion", "tuning",
        "chiptuning", "remapeo", "downpipe",
        "escape deportivo", "suspension deportiva",
        "coilovers", "resortes", "amortiguadores deportivos",
        # ── Diagnostics & ECU ──
        "scanner", "escaner", "diagnostico automotriz", "diagnosticar",
        "ecu", "obd", "obd2", "codigo de falla", "codigos de falla",
        "check engine", "testigo", "luz de advertencia",
        "escaneo", "lectura de codigos", "parametros",
        ...
        "afinamiento", "alineacion",
        "balanceo",
        "intervalo de servicio", "programa de mantenimiento",
        "mantenimiento preventivo", "mantenimiento correctivo",
        "mantenimiento predictivo",
        # ── Tools ──
        "herramienta", "herramientas",
        "llave", "llaves", "torquimetro",
        "multimetro", "osciloscopio",
        "jack", "gato", "crick",
        # ── Accessories & components ──
        "accesorio", "accesorios",
        "kit", "kit de embrague", "kit de frenos",
        "kit de distribucion", "kit de rodamientos",
        "rodamiento", "rodamientos", "collarin",
        "tensor", "polea",
        "resorte",
        # ── Insurance, financing, commerce ──
